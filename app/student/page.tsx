...
export default function StudentDashboard() {
  ...
  const computedStats = useMemo<DashboardStats>(
    () => ({
      ...
      attendanceRate:
        ...
        normalizePercentage(student?.attendance_rate ?? null),
      activeCourses:
  ...
  const persistStudentSession = useCallback(
    (payload: Student, persist: boolean) => {
  ...
  const handleFirstLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showAlert({
          message:
            'Profile updated successfully! Welcome to the student portal.',
          type: 'success',
        });
    ...
    try {
      ...
      try {
        ...
        if (logoDataUrl && logoDataUrl.startsWith('data:image')) {
          ...
          console.log(
            `Logo added to PDF at (10, 10) with size ${logoWidth}x${logoHeight}mm`
          );
        } else {
  ...
  const generateCertificationPDF = useCallback(async () => {
    ...
    try {
      ...
      yPos = yPos + 10;
      const purposeText =
        'This certification is issued upon the request of the aforementioned student for Entrance Examination Slated for Cavite State University - Main Campus. This certification is issued for the stated purpose only.';
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(
        'Note: Should there be a need to verify this document?',
        20,
        150
      );
  ...
  const statCards = [
    ...
    },
    {
      ...
      helper:
        computedStats.attendanceRate !== null
          ? 'Last 30 days'
          : 'Attendance data will appear once available',
    },
    {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setFirstLoginForm({ ...firstLoginForm, gender: value })
                      }
                    >
                      <SelectTrigger>
