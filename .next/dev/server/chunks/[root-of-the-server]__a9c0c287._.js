...
"[project]/app/api/teacher/stats/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: classes, error: classesError } = await admin.from('classes').select('id, class_name, section, room, schedule').eq('teacher_id', teacherId).eq('is_active', true);
        if (classesError) {
        ...
        if (classes && classes.length > 0) {
            ...
            const { count, error: enrollmentError } = await admin.from('class_enrollments').select('*', {
                count: 'exact',
                head: true
        ...
        const { count: journalCount } = await admin.from('journal_entries').select('*', {
            count: 'exact',
            head: true
        }).eq('teacher_id', teacherId);
        // Get active announcements
        const { data: announcements } = await admin.from('announcements').select('id, title, content, priority, published_at').eq('is_active', true).or('target_audience.eq.all,target_audience.eq.teachers').lte('published_at', new Date().toISOString()).order('priority', {
            ascending: false
        }).order('published_at', {
            ascending: false
        }).limit(5);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a9c0c287._.js.map