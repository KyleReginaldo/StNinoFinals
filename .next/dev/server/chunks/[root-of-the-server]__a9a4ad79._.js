...
"[project]/app/api/admin/chart-data/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { count: totalStudents } = await supabaseAdmin.from('users').select('*', {
            count: 'exact',
            head: true
        }).eq('role', 'student');
        const weeklyAttendance = await Promise.all(days.map(async ({ label, date })=>{
            const { data } = await supabaseAdmin.from('attendance_records').select('user_id').gte('scan_datetime', `${date}T00:00:00`).lte('scan_datetime', `${date}T23:59:59`);
        ...
        const gradeApprovals = [
            ...
            {
                name: 'Approved',
                value: gradeCounts.approved,
                fill: '#22c55e'
            },
            {
                name: 'Rejected',
                value: gradeCounts.rejected,
                fill: '#ef4444'
            }
        ];
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a9a4ad79._.js.map