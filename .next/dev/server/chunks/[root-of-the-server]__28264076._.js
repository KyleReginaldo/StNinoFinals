...
"[project]/app/api/admin/parents/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        // Fetch all parents
        const { data: parents, error } = await supabaseAdmin.from('users').select('*').eq('role', 'parent').order('created_at', {
            ascending: false
        });
        if (error) {
        ...
        const parentsWithChildren = await Promise.all((parents || []).map(async (parent)=>{
            const { data: relationships } = await supabaseAdmin.from('user_relationships').select(`
            id,
            relationship_type,
            related_user_id,
...
async function POST(request) {
    try {
        ...
        const { first_name, last_name, middle_name, email, phone_number, address, password } = body;
        const supabaseAdmin = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseAdmin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getSupabaseAdmin"])();
        // Create auth user
...
//# sourceMappingURL=%5Broot-of-the-server%5D__28264076._.js.map