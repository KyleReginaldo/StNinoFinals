...
async function GET() {
    try {
        ...
        const { count: studentsCount, error: studentsError } = await supabaseAdmin.from('users').select('*', {
            count: 'exact',
            head: true
        }).eq('role', 'student');
        // Fetch teachers count
        const { count: teachersCount, error: teachersError } = await supabaseAdmin.from('users').select('*', {
            count: 'exact',
            head: true
        }).eq('role', 'teacher');
        // Fetch parents count
        const { count: parentsCount, error: parentsError } = await supabaseAdmin.from('users').select('*', {
            count: 'exact',
            head: true
        }).eq('role', 'parent');
...
//# sourceMappingURL=%5Broot-of-the-server%5D__cd58a794._.js.map