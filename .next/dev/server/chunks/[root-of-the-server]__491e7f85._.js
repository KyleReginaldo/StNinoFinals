...
async function GET(request) {
    try {
        ...
        const { data: userClasses, error: userClassesError } = await admin.from('user_classes').select('class_id').eq('user_id', teacherId).eq('membership_type', 'teacher');
        if (userClassesError) {
        ...
        // Fetch class details
        const { data: classes, error: classesError } = await admin.from('classes').select('*').in('id', classIds).eq('is_active', true).order('school_year', {
            ascending: false
        }).order('semester', {
            ascending: false
        });
        ...
        const classesWithStudents = await Promise.all((classes || []).map(async (classItem)=>{
            const { data: studentEnrollments } = await admin.from('user_classes').select('user_id').eq('class_id', classItem.id).eq('membership_type', 'student');
            const studentIds = studentEnrollments?.map((se)=>se.user_id) || [];
            let students = [];
            if (studentIds.length > 0) {
                const { data: studentsData } = await admin.from('users').select('id, first_name, last_name, student_number, grade_level, section').in('id', studentIds).eq('role', 'student');
...
//# sourceMappingURL=%5Broot-of-the-server%5D__491e7f85._.js.map