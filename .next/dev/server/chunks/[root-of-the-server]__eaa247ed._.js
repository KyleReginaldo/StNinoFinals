...
"[project]/app/api/teacher/chart-data/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: gradeRows } = await admin.from('grades').select('status').eq('teacher_id', teacherId);
        const counts = {
            pending: 0,
            approved: 0,
            rejected: 0
        ...
        // Students per class
        const { data: classes } = await admin.from('classes').select('id, class_name').eq('teacher_id', teacherId).eq('is_active', true);
        const studentsPerClass = await Promise.all((classes ?? []).map(async (c)=>{
            const { count } = await admin.from('class_enrollments').select('*', {
                count: 'exact',
                head: true
...
//# sourceMappingURL=%5Broot-of-the-server%5D__eaa247ed._.js.map