...
"[project]/app/api/auth/detect-role/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const { data: user } = await admin.from('users').select('role').ilike('email', emailLower).maybeSingle();
        if (user?.role) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                role: user.role
            });
        ...
        const { data: teachers } = await admin.from('teachers').select('email').ilike('email', emailLower).limit(1);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0e43251b._.js.map